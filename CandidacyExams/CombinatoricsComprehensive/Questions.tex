...
\begin{document}
	...
	%\begin{thm}[Multicolour Ramsey]
	%	Let $q_1, q_2, \ldots, q_t$ be positive integers.  Then $n := R(q_1, q_2, \ldots, q_t)$ is the smallest integer such that for some $i \in [t]$, any $t$-colouring of a complete graph of order $n$, denoted $C: E(K_n) \rightarrow [t]$, contains a $K_{q_i}$ whose edges are all coloured $i$.
	%\end{thm}
